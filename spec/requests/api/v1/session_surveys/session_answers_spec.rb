...
describe 'POST api/v1/session_surveys/answers' do
  ...
  let(:session_survey_question) { create(:session_survey_question, type:) }
  ...
  subject do
    post answers_api_v1_session_surveys_path, params:, headers: auth_headers, as: :json
  end
  ...
  context 'when the user has an user session' do
    before { create(:user_session, user:, checked_in: true) }
