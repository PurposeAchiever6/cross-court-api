...
describe ReferralCashPayments::Paid do
  describe '.call' do
    let!(:referral_cash_payment) { create(:referral_cash_payment, status:) }
    let(:status) { :pending }

    subject { ReferralCashPayments::Paid.call(referral_cash_payment:) }

    it { expect { subject }.to change { referral_cash_payment.reload.status }.to('paid') }
